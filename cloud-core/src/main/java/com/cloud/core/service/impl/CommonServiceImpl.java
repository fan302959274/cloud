...
public class CommonServiceImpl implements CommonService {
    ...
    @Value("${redis.master.number}")
    String clusterNum;

    @Override
    public String getBusinessId() {
        RedisAtomicLong counter = new RedisAtomicLong("businessdata", redisTemplate.getConnectionFactory());
        return DateUtil.format(new Date(), DateUtil.YYYYMMDD) + StringUtils.leftPad(counter.getAndAdd(Long.parseLong(clusterNum)) + "", 10, "0");
    }
    ...
    public String getSerialNo(String type) {
        ...
        String result = (StringUtils.isBlank(type) ? "serial" : type) + StringUtils.leftPad(counter.getAndAdd(Long.parseLong(clusterNum)) + "", 10, "0");
        return result;
    }
}